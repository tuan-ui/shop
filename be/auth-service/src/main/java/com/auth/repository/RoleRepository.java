...
public interface RoleRepository extends JpaRepository<Role, String> {
	...
	@Query("""
        SELECT new com.auth.dto.response.RoleResponse(
            r.id,
            r.roleName,
            r.roleCode,
            r.roleDescription,
    ...
    @Query("SELECT r.roleName FROM Role r Join UserRoles ur on r.id = ur.id.roleId WHERE ur.id.userId = :id AND r.isDeleted = false")
    List<String> findLstRoleNameByUserId(UUID id);
}
